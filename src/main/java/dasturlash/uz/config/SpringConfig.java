...
public class SpringConfig {
    ...
    public AuthenticationProvider authenticationProvider() {
        ...
        UserDetails admin = User.builder()
                .username("admin")
                .password("{bcrypt}"+pass2) //pass2 bcrypt orqali kodlash
                .roles("USER")
                .build(); //ecurity ga defoult user ni ruchnoy yarattik

        final DaoAuthenticationProvider daoAuthenticationProvider = new DaoAuthenticationProvider(); //<- shu obyekt orqali
        daoAuthenticationProvider.setUserDetailsService(new InMemoryUserDetailsManager(user, admin)); //va InM.. xotiraga joylab qoydik /baza/file bolMumkun
    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        ...
        http.authorizeHttpRequests(authorizationManagerRequestMatcherRegistry -> {
            authorizationManagerRequestMatcherRegistry
                    ...
                    .requestMatchers("/task/my/all").hasAnyRole("USER","ADMIN") //murojat qlyotkan url USER va ADMIN ga ruxsat
